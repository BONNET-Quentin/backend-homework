...
def create_note():
    ...
    # A UTILISER AVEC http -I :5000/api/notes title="MMC" content="reviser la fiche" done="False" par exemple
    # OU http -I post :5000/api/notes title="MMC" content="reviser la fiche" done="False"
    # OU http -I post :5000/api/notes title="MMC" content="reviser la fiche" sans paramètre done
    ...
    try:
        ...
        done = normalize_bool(parameters['done']) if 'done' in parameters else False
        # temporary
        new_note = Note(title=title, content=content, done=done)
        db.session.add(new_note)
        db.session.commit()
        return dict(id=new_note.id, message="Note created"), 201
